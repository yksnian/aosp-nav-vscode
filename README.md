...
Exclusion lists use **append** semantics by default: your entries are addedafter the built-in defaults (which already cover every known stub family).Set `aosp-nav.excludeMerge` … i.e. `aosp-nav.excludeMerge` to `"replace"` todiscard the defaults.
...
```bash
npm install
npm run compile# F5 in VSCode (launch config included) → Extension Development Hostnpx
